...
template<typename T>
Tensor<T> conv2d(Tensor<T> &in_t, int out_size, int kernel_size, const Tensor<T> &filters, const Tensor<T> &biases)
{
    ...
    int out_rows = in_t.rows() - window_row_margin * 2;
    int out_cols = in_t.cols() - window_col_margin * 2;

    // number of output channels
    int out_channels = filters.channels();

    int groups = in_t.groups();
    //int rows = in_t.rows();
    //int cols = in_t.cols();
    ...
    //int n_cols = cols - window_col_size + 1;
    T kernel[window_row_size * window_col_size * in_t.channels()];
                ...
                for (int out_ch = 0; out_ch < out_channels; out_ch++) {
                    ...
                    for (int kr = r; kr < r + window_row_size; kr++) {
                        for (int kc = c; kc < c + window_col_size; kc++) {
                            for (int in_ch = 0; in_ch < in_t.channels(); in_ch++) {
